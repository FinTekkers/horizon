...
test('passes the farm log body through with the offset', async () => {
  ...
  const res = await inject({ method: 'GET', url: '/api/runs/7/log?offset=12' })
...
test('a run the farm does not know is a 404', async () => {
  ...
  const res = await inject({ method: 'GET', url: '/api/runs/999/log' })
...
test('a negative offset is rejected at the schema layer', async () => {
  ...
  const res = await inject({ method: 'GET', url: '/api/runs/7/log?offset=-1' })
  assert.equal(res.statusCode, 400)
})

test('401s without a session cookie (HZ-21)', async () => {
  farmResponse = { status: 200, body: {} }
