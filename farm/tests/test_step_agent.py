...
def test_two_exhausted_attempts_then_a_successful_run_converges_with_continuation_history(tmp_path, monkeypatch):
    ...
    assert result["artifacts"]["branch"] == "horizon/t-1"
    log_text = origin_log(origin)
    subjects = log_text.strip().splitlines()
    # History is additive — both checkpoint commits survive alongside the
    # normal finalize commit, never squashed or discarded.
    assert sum(step_agent.CHECKPOINT_MARKER in s for s in subjects) == 2
    assert any("Test item" in s for s in subjects)
    for name in ("part_a.txt", "part_b.txt", "part_c.txt"):
        shown = subprocess.run(
            ["git", "--git-dir", str(origin), "show", f"horizon/t-1:{name}"],
            capture_output=True,
            text=True,
            check=True,
        ).stdout
